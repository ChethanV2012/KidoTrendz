...
// Dummy price formatting component
const PriceFormat = ({ amount, className }) => (
  <span className={className}>${amount?.toLocaleString()}</span>
);

// Dummy container (replace with your actual Container if needed)
...
const Banner = () => {
  ...
  useEffect(() => {
    fetch("/api/banners")
      ...
      .catch(() => {
        // fallback: sample banner data (remove this in production)
        setBannerData([
          {
    ...
    >
      <Slider ref={sliderRef} {...settings}>
        {bannerData.map((item, index) => (
          <div key={index} className="relative h-[70vh] min-h-[500px] max-h-[700px]">
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <svg className="relative z-10 w-4 lg:w-5 h-4 lg:h-5 transition-all duration-300 group-hover:translate-x-2 group-hover:text-white"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
